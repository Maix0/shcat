	...
	const char *field_name)
{
	...
	if (is_visible)
	{
			...
			self.ptr->size.bytes > 0)
		{
			...
			cursor += ts_subtree__write_char_to_string(
				*writer, limit, self.ptr->lookahead_char);
		}
		else
		{
			t_symbol symbol =
	...
	if (ts_subtree_child_count(self))
	{
		...
		for (t_u32 i = 0; i < self.ptr->child_count; i++)
		{
			...
			{
				cursor +=
					ts_subtree__write_to_string(child, *writer, limit, language,
			...
			else
			{
				...
				{
					if (!map->inherited &&
						map->child_index == structural_child_index)
					{
				...
				cursor += ts_subtree__write_to_string(
					child, *writer, limit, language, include_all,
					subtree_alias_symbol, subtree_alias_is_named,
					child_field_name);
