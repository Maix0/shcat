...
static void ts_parser__external_scanner_deserialize(TSParser *self, Subtree external_token)
{
	...
	{
		data = ts_external_scanner_state_data(&external_token.ptr->inner.external_scanner_state);
		length = external_token.ptr->inner.external_scanner_state.length;
	}
...
static Subtree ts_parser__lex(TSParser *self, StackVersion version, TSStateId parse_state)
{
	...
	else
	{
		...
		if (found_external_token)
		{
			...
			ts_external_scanner_state_init(&mut_result.ptr->inner.external_scanner_state, self->lexer.debug_buffer, external_scanner_state_len);
									  ...
									  uint16_t production_id, bool is_fragile, bool end_of_non_terminal_extra)
{
	...
	for (uint32_t i = 0; i < pop.size; i++)
	{
		...
		parent.ptr->inner.non_terminal.dynamic_precedence += dynamic_precedence;
	...
	for (uint32_t i = 0; i < pop.size; i++)
	{
		...
		for (uint32_t j = trees.size - 1; j + 1 > 0; j--)
		{
			...
			if (!ts_subtree_extra(tree))
			{
				...
				root = ts_subtree_from_mut(ts_subtree_new_node(ts_subtree_symbol(tree), &trees, tree.ptr->inner.non_terminal.production_id, self->language));
