...
bool ts_stack_print_dot_graph(Stack *self, const TSLanguage *language, FILE *f)
{
	...
	for (uint32_t i = 0; i < self->heads.size; i++)
	{
		...
		if (head->last_external_token.ptr)
		{
			const ExternalScannerState *state = &head->last_external_token.ptr->inner.external_scanner_state;
	...
	while (!all_iterators_done)
	{
		...
		for (uint32_t i = 0; i < self->iterators.size; i++)
		{
			...
			else if (node->link_count == 1 && node->links[0].subtree.ptr && ts_subtree_extra(node->links[0].subtree))
			{
			...
			for (int j = 0; j < node->link_count; j++)
			{
				...
				else
				{
					...
					if (quoted)
						fprintf(f, "'");
					fprintf(f, "\"");
					fprintf(f, "labeltooltip=\"error_cost: %u\ndynamic_precedence: %" PRId32 "\"", ts_subtree_error_cost(link.subtree),
