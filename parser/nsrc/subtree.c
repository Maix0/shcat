...
Subtree ts_subtree_new_error(SubtreePool *pool, int32_t lookahead_char, Length padding, Length size, uint32_t bytes_scanned,
							 TSStateId parse_state, const TSLanguage *language)
{
	...
	data->inner.lookahead_char = lookahead_char;
	return result;
}
...
MutableSubtree ts_subtree_clone(Subtree self)
{
	...
	{
		result->inner.external_scanner_state = ts_external_scanner_state_copy(&self.ptr->inner.external_scanner_state);
	}
	result->ref_count = 1;
...
void ts_subtree_balance(Subtree self, SubtreePool *pool, const TSLanguage *language)
{
	...
	while (pool->tree_stack.size > 0)
	{
		...
		if (tree.ptr->inner.non_terminal.repeat_depth > 0)
		{
...
void ts_subtree_summarize_children(MutableSubtree self, const TSLanguage *language)
{
	...
	self.ptr->inner.non_terminal.named_child_count = 0;
	self.ptr->inner.non_terminal.visible_child_count = 0;
	self.ptr->error_cost = 0;
	self.ptr->inner.non_terminal.repeat_depth = 0;
	self.ptr->inner.non_terminal.visible_descendant_count = 0;
	self.ptr->has_external_tokens = false;
	self.ptr->depends_on_column = false;
	self.ptr->has_external_scanner_state_change = false;
	self.ptr->inner.non_terminal.dynamic_precedence = 0;

	uint32_t		structural_index = 0;
	const TSSymbol *alias_sequence = ts_language_alias_sequence(language, self.ptr->inner.non_terminal.production_id);
	...
	for (uint32_t i = 0; i < self.ptr->child_count; i++)
	{
		...
		if (self.ptr->symbol == ts_builtin_sym_error || self.ptr->symbol == ts_builtin_sym_error_repeat)
		{
			if (!ts_subtree_extra(child) && !(ts_subtree_is_error(child) && grandchild_count == 0))
			{
				...
				{
					self.ptr->error_cost += ERROR_COST_PER_SKIPPED_TREE * child.ptr->inner.non_terminal.visible_child_count;
				}
			}
		}

		self.ptr->inner.non_terminal.dynamic_precedence += ts_subtree_dynamic_precedence(child);
		self.ptr->inner.non_terminal.visible_descendant_count += ts_subtree_visible_descendant_count(child);

		if (alias_sequence && alias_sequence[structural_index] != 0 && !ts_subtree_extra(child))
		{
			self.ptr->inner.non_terminal.visible_descendant_count++;
			self.ptr->inner.non_terminal.visible_child_count++;
			if (ts_language_symbol_metadata(language, alias_sequence[structural_index]).named)
			{
				self.ptr->inner.non_terminal.named_child_count++;
			}
		}
		...
		{
			self.ptr->inner.non_terminal.visible_child_count += child.ptr->inner.non_terminal.visible_child_count;
			self.ptr->inner.non_terminal.named_child_count += child.ptr->inner.non_terminal.named_child_count;
		}
	...
	if (self.ptr->child_count > 0)
	{
		...
		self.ptr->inner.non_terminal.first_leaf.symbol = ts_subtree_leaf_symbol(first_child);
		self.ptr->inner.non_terminal.first_leaf.parse_state = ts_subtree_leaf_parse_state(first_child);
		...
		if (self.ptr->child_count >= 2 && !self.ptr->visible && !self.ptr->named && ts_subtree_symbol(first_child) == self.ptr->symbol)
		{
			...
			{
				self.ptr->inner.non_terminal.repeat_depth = ts_subtree_repeat_depth(first_child) + 1;
			}
			else
			{
				self.ptr->inner.non_terminal.repeat_depth = ts_subtree_repeat_depth(last_child) + 1;
			}
		}
	}
}
...
void ts_subtree_release(SubtreePool *pool, Subtree self)
{
	...
	while (pool->tree_stack.size > 0)
	{
		...
		else
		{
			...
			{
				ts_external_scanner_state_delete(&tree.ptr->inner.external_scanner_state);
			}
										  ...
										  TSSymbol alias_symbol, bool alias_is_named, const char *field_name)
{
	...
	if (is_visible)
	{
		...
		if (ts_subtree_is_error(self) && ts_subtree_child_count(self) == 0 && self.ptr->size.bytes > 0)
		{
			...
			cursor += ts_subtree__write_char_to_string(*writer, limit, self.ptr->inner.lookahead_char);
		}
		else
		{
	...
	if (ts_subtree_child_count(self))
	{
		...
		ts_language_field_map(language, self.ptr->inner.non_terminal.production_id, &field_map, &field_map_end);
...
void ts_subtree__print_dot_graph(const Subtree *self, uint32_t start_offset, const TSLanguage *language, TSSymbol alias_symbol, FILE *f)
{
	...
	if (ts_subtree_is_error(*self) && ts_subtree_child_count(*self) == 0 && self->ptr->inner.lookahead_char != 0)
	{
		fprintf(f, "\ncharacter: '%c'", self->ptr->inner.lookahead_char);
	}

	fprintf(f, "\"]\n");

	uint32_t child_start_offset = start_offset;
	uint32_t child_info_offset = language->max_alias_sequence_length * ts_subtree_production_id(*self);
...
const ExternalScannerState *ts_subtree_external_scanner_state(Subtree self)
{
	...
	{
		return &self.ptr->inner.external_scanner_state;
	}
	else
	{
		return &empty_state;
