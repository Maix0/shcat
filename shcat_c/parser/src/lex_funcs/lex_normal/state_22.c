...
bool	lex_normal_s110(t_lexer *lexer, t_lexer_state *s)
{
	...
	if (s->lookahead == '\r')
		return (lex_advance(109, lexer, s));
...
{
	if (s->lookahead == '\n')
		return (lex_advance(917, lexer, s));
	return (lex_end_state(lexer, s));
}
...
bool	lex_normal_s112(t_lexer *lexer, t_lexer_state *s)
{
	if (s->lookahead == '\n')
		return (lex_advance(917, lexer, s));
	if (s->lookahead == '\r')
		return (lex_advance(111, lexer, s));
	if (((s->lookahead >= '\t' && s->lookahead <= '\f') || \
	s->lookahead == ' '))
...
{
	if (s->lookahead == '\n')
		return (lex_advance(934, lexer, s));
	return (lex_end_state(lexer, s));
}
...
bool	lex_normal_s114(t_lexer *lexer, t_lexer_state *s)
{
	if (s->lookahead == '\n')
		return (lex_advance(934, lexer, s));
	if (s->lookahead == '\r')
		return (lex_advance(113, lexer, s));
	if (((s->lookahead >= '\t' && s->lookahead <= '\f') || \
	s->lookahead == ' '))
