...
#define LOG(...)                                                               \
	...
	{                                                                          \
		snprintf(self->lexer.debug_buffer,                                     \
				 TREE_SITTER_SERIALIZATION_BUFFER_SIZE, __VA_ARGS__);          \
		ts_parser__log(self);                                                  \
	}
...
#define LOG_LOOKAHEAD(symbol_name, size)                                       \
	...
	{                                                                          \
		...
		snprintf(buf + off, TREE_SITTER_SERIALIZATION_BUFFER_SIZE - off,       \
				 ", size:%u", size);                                           \
		ts_parser__log(self);                                                  \
	}
...
#define LOG_TREE(tree)                                                         \
	if (self->dot_graph_file)                                                  \
	{                                                                          \
		ts_subtree_print_dot_graph(tree, self->language,                       \
								   self->dot_graph_file);                      \
		fputs("\n", self->dot_graph_file);                                     \
	}
							  ...
							  TSStateId parse_state)
{
	...
	for (;;)
	{
		...
		if (lex_mode.external_lex_state != 0)
		{
			LOG("lex_external state:%d, row:%u, column:%u",
				lex_mode.external_lex_state, current_position.extent.row,
					...
					LOG("ignore_empty_external_token symbol:%s",
						SYM_NAME(
							self->language->external_scanner
								.symbol_map[self->lexer.data.result_symbol]))
					found_token = false;
				}
			}
	...
	LOG_LOOKAHEAD(SYM_NAME(ts_subtree_symbol(result)),
									 ...
									 TableEntry *table_entry)
{
	...
	while ((result = reusable_node_tree(&self->reusable_node)).ptr)
	{
		...
		{
			LOG("cant_reuse_node symbol:%s, first_leaf_symbol:%s",
				TREE_NAME(result), SYM_NAME(leaf_symbol));
...
static bool ts_parser__select_tree(TSParser *self, Subtree left, Subtree right)
{
	...
	{
		LOG("select_higher_precedence symbol:%s, prec:%" PRId32
			", over_symbol:%s, other_prec:%" PRId32,
			TREE_NAME(right), ts_subtree_dynamic_precedence(right),
			TREE_NAME(left), ts_subtree_dynamic_precedence(left));
		return true;
	}
	...
	if (summary && !ts_subtree_is_error(lookahead))
	{
		for (unsigned i = 0; i < summary->size; i++)
		{
										...
										ts_subtree_symbol(lookahead)))
			{
				...
				{
					did_recover = true;
					LOG("recover_to_previous state:%u, depth:%u", entry.state,
						depth);
					LOG_STACK();
					break;
				}
							   ...
							   bool allow_node_reuse)
{
	...
	for (;;)
	{
		...
		for (uint32_t i = 0; i < table_entry.action_count; i++)
		{
			...
			case TSParseActionTypeReduce: {
				...
				LOG("reduce sym:%s, child_count:%u",
					SYM_NAME(action.reduce.symbol), action.reduce.child_count);
				...
				self->language->keyword_capture_token)
		{
			...
			if (table_entry.action_count > 0)
			{
				LOG("switch from_keyword:%s, to_word_token:%s",
					TREE_NAME(lookahead),
...
TSTree *ts_parser_parse(TSParser *self, const TSTree *old_tree, TSInput input)
{
	...
	do
	{
			 ...
			 version++)
		{
			...
			{
				LOG("process version:%u, version_count:%u, state:%d, row:%u, "
					"col:%u",
